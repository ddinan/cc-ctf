...
namespace CTF
{
    public sealed class CmdLobby : Command2
    {
        ...
        {
            p.Message("Active Lobbies:");

            foreach (var lobby in LobbyManager.lobbies)
            {
                p.Message($"Lobby ID: {lobby.LobbyId}, Players: {lobby.Players.Count}, Game Running: {lobby.IsGameRunning}");
            }
        }
        ...
        {
            if (args.Length < 2) {
                p.Message("Please specify the lobby to join with /lobby join [lobby id]");
                return;
            }
        ...
        {
            p.Message("%T/Lobby list %H- Lists all available game lobbies.");
            p.Message("%T/Lobby join [lobby] %H- Joins an existing game lobby, if permitted.");
            p.Message("%T/Lobby create %H- Creates a new game lobby.");
        }
    }
}